...
public class SearchBreedByNameTest {
    private final TheCatApiClientPort theCatApiClientPort = Mockito.mock(TheCatApiClientPort.class);
    private final SearchBreedByNameUseCase searchBreedByNameUseCase = new SearchBreedByNameUseCase(theCatApiClientPort);

    @Test
    public void testExecute() {
        // Crear un comando de prueba
        SearchBreedByNameCommand command = SearchBreedByNameCommand
                .builder()
        ...
        when(theCatApiClientPort.searchBreadByName(command.getName())).thenReturn(Flux.just(cardItem));

        // Llamar al método execute
        Either<UseCaseErrorResponse, SearchBreedByNameInformation> result = searchBreedByNameUseCase.execute(command);

        // Verificar que el método getBreads del puerto se llamó una vez
        verify(theCatApiClientPort, times(1)).searchBreadByName("Abyssinian");

        // Asegurarse de que el resultado es el flujo de razas de gato
        assertNotNull(result.get().getData(), "El campo data es nulo");
        assertEquals("Abyssinian", result.get().getData().getBreads().get(0).getName());
